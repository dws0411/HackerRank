...
int  sum (int count,...) {
    int i,sum;
    va_list vp; //stdarg.h提供的数据类型，用于函数可变参数
    va_start(vp, count);            //va_start函数使vp指向第一个可选参数
    for(i=0; i<count;i++){
        sum += va_arg(vp, int);     //va_arg返回参数列表中当前参数，并使vp指向下一参数
    };
    va_end(vp);
    return sum;
}
...
int min(int count,...) {
    ...
    min = va_arg(vp, int);
    int temp;
    for(i =1;i<count;i++){      //由于已调用va_arg一次，故此处i从1开始即可
        temp = va_arg(vp, int);
        if(min > temp){
            min = temp;
        }
